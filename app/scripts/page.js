      ...
      <main style={{ maxWidth: '1000px', margin: '0 auto', padding: '48px 24px 80px' }}>
          ...
          <section key={cat.label} id={`cat-${cat.label.toLowerCase()}`} style={{ marginBottom: '72px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {cat.scripts.map(script => (
                <article key={script.title} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '14px', padding: '28px 32px', transition: 'box-shadow 0.2s' }}
                  onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 20px rgba(26,20,15,0.08)'}
        ...
        <section id="sources" style={{ marginBottom: '0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
            {scriptSources.map(s => (
              <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#fff', border: '1px solid var(--border)', borderRadius: '12px', padding: '18px 20px', textDecoration: 'none', transition: 'border-color 0.2s' }}
