  ...
  return (
    <>
      <nav style={{
        ...
        background: scrolled ? 'rgba(26,81,46,0.97)' : 'rgba(26,81,46,0.92)',
        backdropFilter:'blur(24px)', WebkitBackdropFilter:'blur(24px)',
        borderBottom:'1px solid rgba(98,168,30,0.25)',
          ...
          <div className="nav-desktop" style={{ display:'flex',alignItems:'center',gap:'2px',flex:1,paddingLeft:'22px' }}>
            <div ref={learnRef} style={{ position:'relative' }} onMouseEnter={openLearn} onMouseLeave={scheduleClose}>
              <button style={{
                ...
                color: learnActive||learnOpen ? 'var(--green)' : 'rgba(244,249,240,0.70)',
            ...
            {user ? (
              <>
                <Link href="/profile" style={{ textDecoration:'none' }}>
                  <button className="btn-ghost" style={{ fontSize:'13px',padding:'6px 14px', display:'flex', alignItems:'center', gap:'6px', color:'rgba(244,249,240,0.80)', borderColor:'rgba(242,237,228,0.25)' }}>
                ...
                <button onClick={handleSignOut} className="btn-ghost" style={{ fontSize:'13px',padding:'6px 14px', color:'rgba(244,249,240,0.80)', borderColor:'rgba(242,237,228,0.25)' }}>Sign out</button>
              </>
            ) : (
              <>
                <Link href="/auth" style={{ textDecoration:'none' }}>
                  <button className="btn-ghost" style={{ fontSize:'13px',padding:'6px 14px', color:'rgba(244,249,240,0.80)', borderColor:'rgba(242,237,228,0.25)' }}>Sign in</button>
                </Link>
                <Link href="/auth" style={{ textDecoration:'none' }}>
...
function NavLink({ href, active, children }) {
  return (
    <Link href={href} style={{ textDecoration:'none' }}>
      <span style={{ display:'inline-block',padding:'6px 10px',borderRadius:'8px',fontSize:'14px',fontWeight:active?'600':'400',color:active?'var(--green)':'rgba(244,249,240,0.70)',background:active?'rgba(74,140,140,0.18)':'transparent',transition:'background 0.15s,color 0.15s',cursor:'pointer' }}
        onMouseEnter={e=>{if(!active){e.currentTarget.style.background='var(--green-pale)';e.currentTarget.style.color='var(--green)'}}}
        onMouseLeave={e=>{if(!active){e.currentTarget.style.background='transparent';e.currentTarget.style.color='rgba(244,249,240,0.70)'}}}
      >{children}</span>
    </Link>
  )
}
