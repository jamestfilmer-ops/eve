...
function TimelineTab({ scenes }) {
  const ACT_COLORS = {
    1: { bg: '#E4F0F0', border: '#6AAF3D', label: 'Act 1' },
    2: { bg: '#FFF7ED', border: '#8BA5A0', label: 'Act 2' },
...
function ThemesMapTab({ projectId, scenes, themes, setThemes, toast }) {
  ...
  const THEME_COLORS = {
    theme:    { bg: '#4A8C8C', text: '#fff', border: '#1A3009' },
    motif:    { bg: '#8BA5A0', text: '#fff', border: '#7A4C07' },
...
const SM_NODE = {
  ...
  theme:     { w: 128, h: 40, rx: 6,  bg: '#4A8C8C', stroke: '#1A3009', fg: '#fff' },
  subplot:   { w: 136, h: 44, rx: 6,  bg: '#8BA5A0', stroke: '#7A4C07', fg: '#fff' },
}

const ACT_ZONE = {
  1: { fill: 'rgba(239,246,231,0.7)', stroke: '#9ECECE' },
      ...
      {scenes.length === 0 && characters.length === 0 ? (
        ...
        <>
            ...
            >
              <defs>
                ...
                <marker id="sm-arrow-amber" markerWidth="9" markerHeight="7" refX="9" refY="3.5" orient="auto">
                  <polygon points="0 0, 9 3.5, 0 7" fill="#8BA5A0" />
                </marker>
              </defs>
              ...
              {edges.map(edge => {
                ...
                const color     = isSel ? 'var(--green)' : isActBrk ? '#8BA5A0' : '#94A3B8'
          ...
          {selected?.startsWith('edge-') && (
            <div style={{ marginTop: '12px', padding: '10px 16px', background: '#EEF4F3', borderRadius: '8px', border: '1px solid var(--amber-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '18px', marginTop: '14px', flexWrap: 'wrap' }}>
            {[
              ...
              { color: '#4A8C8C', label: 'Theme' },
              { color: '#8BA5A0', label: 'Subplot' },
            ].map(({ color, label }) => (
