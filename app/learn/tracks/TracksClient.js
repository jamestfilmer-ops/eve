...
export default function TracksClient({ tracks }) {
  const [openTrack, setOpenTrack] = useState('absolute-beginner')
        ...
        return (
          <section key={track.id} style={{ background:'#fff', border:'1px solid var(--border)', borderRadius:'16px', overflow:'hidden' }}>
            ...
            <button
              ...
              style={{ width:'100%', background:'none', border:'none', cursor:'pointer', textAlign:'left', padding:'22px 28px 18px' }}
            >
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'16px' }}>
                ...
                {/* Chevron */}
                <div style={{ flexShrink:0, width:'28px', height:'28px', borderRadius:'50%', background:'var(--off-white)', border:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', marginTop:'4px', transition:'background 0.15s' }}>
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ transition:'transform 0.25s var(--ease-out)', transform: isOpen ? 'rotate(180deg)' : 'none' }}>
                    <path d="M2 4l4 4 4-4" stroke="var(--text-mid)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
              </div>
            </button>
