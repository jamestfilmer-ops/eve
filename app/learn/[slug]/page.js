...
const lessons = {
  ...
  'plant-payoff': {
    title: 'Plant and payoff: the architecture of inevitability',
    category: 'Theme',
    time: '7 min',
    level: 'Intermediate',
    intro: 'A payoff without a plant feels like cheating. A plant without a payoff is dead weight. The craft is in the burial — making the plant invisible until the payoff retroactively illuminates it.',
    body: [
      { type: 'p', text: 'Chekhov\'s rule is usually summarized as: if there\'s a gun on the wall in Act 1, it must fire in Act 3. The reverse matters just as much — if the gun fires in Act 3, it must be on the wall in Act 1. This applies to everything that requires dramatic justification: a skill, a piece of information, an object, a relationship, a character trait. Anything that resolves a story problem must first be introduced as an ordinary story element.' },
      { type: 'rule', text: 'The test for a plant is retroactivity. After the payoff lands, the audience should be able to look back and see the plant was always there. If they have to take your word for it — if the evidence was not actually in the story — you have a cheat, not a plant.' },
      { type: 'h3', text: 'How to bury a plant' },
      { type: 'p', text: 'The plant must appear to be doing something else entirely. The gun on the wall is not on the wall because it will be fired. It\'s there because the character is a hunter — the gun tells us who they are, right now, in this scene. It belongs for other reasons. The fact that it will matter again later is invisible on first reading. When the payoff comes, the audience thinks: of course. They registered it without knowing why.' },
      { type: 'p', text: 'In Toy Story, Buzz Lightyear\'s wings are established as a feature of the toy from his first scene — he\'s a space ranger, of course he has wings. They\'re paid off in the climax when Buzz uses them to glide. The plant is buried in character introduction. No one watching the first act is waiting for a wing scene. The payoff retroactively makes the wings feel inevitable.' },
      { type: 'p', text: 'Emotional plants work the same way. A character\'s fear of abandonment is established early — a throwaway line, a behavioral detail, a reaction that seems like character color. It is planted. Later, when that fear drives a catastrophic decision, the audience accepts it as true rather than convenient. The fear was always there. The story just found the moment to activate it.' },
      { type: 'h3', text: 'The double plant' },
      { type: 'p', text: 'The most sophisticated plants pay off more than once. The same object, skill, or piece of information is used twice — once in a smaller way that establishes it as reliable, and once in the climax where its full weight lands. The first payoff trains the audience to accept the plant as part of the story\'s logic. The second payoff, which is larger, inherits that acceptance.' },
      { type: 'p', text: 'In No Country for Old Men, Anton Chigurh\'s coin toss is planted early and pays off mid-story in the gas station scene — we see exactly what it means when he flips the coin. That mid-story payoff is itself a plant for the film\'s final confrontation with Carla Jean, where the coin appears again with full devastating weight. The first payoff made the second one possible.' },
      { type: 'h3', text: 'Thematic plants vs. plot plants' },
      { type: 'p', text: 'Plot plants are the most familiar kind — an object or skill that will solve a concrete problem. Thematic plants are more powerful and harder to see. A line of dialogue that expresses the story\'s central question. An image that appears once in Act 1 and once at the end with completely different meaning. A character trait that seems incidental until the story turns on it.' },
      { type: 'p', text: 'In The Godfather, Michael Corleone\'s declaration early in the film — "that\'s not me, I\'m not like my family" — is a thematic plant. The entire story is the payoff. Every scene in which Michael becomes exactly what he said he was not is charging that plant with more irony, more tragedy. The line pays off not in a single moment but across the entire arc.' },
      { type: 'h3', text: 'The dead plant' },
      { type: 'p', text: 'A dead plant is something you introduced that never pays off. Most first drafts are full of them — details that felt important when you wrote them but that the story moved away from. They confuse the audience because they look like plants. The reader or viewer waits for them to matter. When they don\'t, the story feels unresolved in ways that are hard to name. The draft that feels vaguely "off" often has several dead plants in Act 1.' },
      { type: 'tip', text: 'List every payoff in your story — every moment where a skill, object, relationship, or piece of information solves a problem or delivers an emotional blow. For each one, trace it back: where did you plant it? Is the plant in the story, visible to any reader? If it is not, add it. If you have details in Act 1 that trace to nothing, either cut them or write the payoff they are asking for.' },
    ],
    books: [
      { title: 'Story', author: 'Robert McKee', note: 'McKee\'s analysis of how setup and payoff function across scenes, sequences, and acts is the most systematic treatment of the subject available.' },
    ],
  },
  'motifs': {
  ...
  'scene-turn': {
    ...
    level: 'Beginner',
    body: [
      { type: 'p', text: 'A scene needs to change something. Not necessarily something large — but by the end, the situation should be different from what it was at the beginning. A relationship shifts. Information changes hands. A decision gets made. Power moves from one character to another. Something. A scene that ends in exactly the same position it started has not done anything, regardless of how well the dialogue is written.' },
      { type: 'rule', text: 'The turn is the moment in the scene where the direction changes. Before the turn, we are going one way. After it, something is different. That shift — in power, in knowledge, or in commitment — is what makes a scene a scene rather than a pause.' },
      { type: 'h3', text: 'Three kinds of turn' },
      { type: 'p', text: 'The power turn is the most visible. A scene begins with one character holding the advantage — the interrogator in control of the room, the employer about to fire someone — and ends with the balance reversed. The subject has not changed but the ground has shifted beneath both of them. Power turns produce the clearest dramatic rhythm because the audience can feel the reversal the moment it happens.' },
      { type: 'p', text: 'The information turn is subtler. A scene ends with someone knowing something they did not know at the start — or, more powerfully, with the audience knowing something a character does not. The information changes the meaning of what was said before it arrived. A conversation that seemed like small talk retroactively becomes sinister when the information turn reveals what one character was actually trying to learn. Information turns are the primary mechanism of dramatic irony.' },
      { type: 'p', text: 'The decision turn is the most structural. A character who was uncertain commits. A character who was committed flinches. A character who had a choice makes it and the story cannot go back. Decision turns do not require dialogue — they can happen in a look, a pause, an action the other character does not even notice. Their effect is not felt in the scene but in the scenes that follow, because the decision now shapes everything coming next.' },
      { type: 'h3', text: 'Why scenes feel pointless' },
      { type: 'p', text: 'A scene that ends in the same position it started — same power dynamic, same knowledge, same emotional state, no commitment made — has not moved. It may have excellent writing in it. It may reveal character. But it has not turned, which means it has not done what scenes are for. The reader or viewer senses this without being able to name it. The scene felt good but it also felt like nothing happened.' },
      { type: 'h3', text: 'The double turn' },
      { type: 'p', text: 'Strong scenes often turn twice. The apparent turn comes early — the information arrives, the power shifts, the decision seems made. Then something in the final beats undermines or complicates it. The scene seems to resolve and then opens again. A negotiation that appears settled ends with one line that reopens everything. This double movement — turn, then reversal of the turn — is what creates the scene endings that make audiences need to see what comes next immediately.' },
      { type: 'tip', text: 'For any scene you are working on, identify the turn: is it a power turn, an information turn, or a decision turn? If you cannot identify one, the scene does not have one. Find the moment where something changes — or write it. If the scene has a turn but still feels flat, ask whether the turn comes too early, giving the scene time to breathe after its own resolution. The turn should land close to the end.' },
    ],
  },
  ...
  'dramatic-irony': {
    title: 'Dramatic irony: the secret the audience keeps',
    category: 'Scene Craft',
    time: '7 min',
    level: 'Intermediate',
    intro: 'Dramatic irony occurs when the audience knows something a character doesn\'t. It is one of the oldest and most reliable sources of tension in storytelling — because it weaponizes empathy. We know the danger. The character doesn\'t. We can\'t intervene.',
    body: [
      { type: 'p', text: 'Hitchcock explained it once with a thought experiment. Two people are talking at a table. A bomb explodes. You have fifteen seconds of surprise. But show the audience the bomb first — let them watch the same two people have the same conversation while knowing it\'s there — and you have fifteen minutes of dread. The scene has not changed. The audience\'s knowledge has. That difference is dramatic irony, and it is the single most reliable tool for generating sustained tension in a scene.' },
      { type: 'rule', text: 'Surprise is fifteen seconds. Suspense is everything before it. Give the audience the secret and the scene transforms from a conversation into a countdown.' },
      { type: 'h3', text: 'Three kinds' },
      { type: 'p', text: 'Tragic irony is when a character moves toward a truth that will destroy them, and the audience can see where they are going. Oedipus investigates a murder without knowing he is the murderer. Romeo buys poison because he doesn\'t know Juliet is alive. The audience holds the knowledge the character needs. The helplessness of knowing and being unable to intervene is the engine of tragedy.' },
      { type: 'p', text: 'Comedic irony is when the character misunderstands something the audience sees clearly. The love interest who thinks the hero hates them. The villain who doesn\'t realize the spy is in the room. The gap between what the character believes and what the audience knows generates comedy precisely because the stakes are lower — we know they\'ll be fine, we just can\'t tell them.' },
      { type: 'p', text: 'Moral irony is when the audience knows a character is fundamentally wrong about something important, and watches them make consequential decisions based on that wrongness. Walter White believes he is providing for his family. The audience can see — increasingly clearly — that he is destroying it. Moral irony creates a specific unease: the character is sincere. They are just wrong. And the audience must watch the wrong play out.' },
      { type: 'h3', text: 'Managing the gap' },
      { type: 'p', text: 'Dramatic irony creates a gap between what characters know and what the audience knows. That gap exists in time — it will close at the revelation. Everything inside the gap is colored by the secret. A dinner scene becomes different if the audience knows one person at the table has betrayed the other. The dialogue hasn\'t changed. The information has.' },
      { type: 'p', text: 'Three things you can do with the gap. Widen it: give the audience more information the character doesn\'t have. Hold it: let the character come close to the truth without finding it — they ask the right question, look in the right direction, and then get distracted or choose not to pursue it. Close it: the revelation arrives. Close it too soon and you\'ve wasted the investment. Close it too late and the audience becomes frustrated rather than tense. The timing of the close is a structural decision, not a scene-level one.' },
      { type: 'h3', text: 'The character who almost knows' },
      { type: 'p', text: 'The most effective use of dramatic irony is placing the character on the edge of the truth and then pulling them away from it. They ask the right question. They look at the right evidence. They almost connect the pieces. And then something interrupts them, or they make a wrong inference, or they choose not to believe what they\'re seeing. The audience watches them reach for the thing that would save them — and let it go.' },
      { type: 'p', text: 'This moment, when a character comes close and misses, is called a near-revelation. It does not close the gap — it widens it. The audience\'s dread intensifies because the character had their chance and didn\'t take it. Near-revelations are one of the most effective ways to sustain dramatic irony over a long story without releasing its tension too early.' },
      { type: 'h3', text: 'Sustained versus momentary' },
      { type: 'p', text: 'Some stories sustain dramatic irony across their entire length. The Sixth Sense gives the audience a secret they won\'t fully decode until the film\'s final minutes. Every scene from that point on is colored by the hidden truth, even if the audience can\'t name it exactly. This requires discipline — the gap must be constantly refreshed, the audience must be kept worried, and every scene must give them reasons to stay in the dread. Sustained irony is structural, not just scenic.' },
      { type: 'p', text: 'Momentary irony is used for a single beat. The audience knows the monster is behind the door. They watch the character approach the door. The moment passes — the character opens a different door, or the audience was wrong about the monster. Momentary irony is the basic unit of thriller construction: raise a question, let it build, answer it. The answer either pays off the dread or redirects it. Both moves work. What doesn\'t work is forgetting the dread is there and moving on without closing it.' },
      { type: 'tip', text: 'Go through your story scene by scene. For each, ask: what does the audience know that at least one character in this scene doesn\'t? If the answer is nothing for five consecutive scenes, the audience is experiencing events alongside the characters rather than ahead of them. Give them a secret. It doesn\'t have to be large — even a small advantage in knowledge changes how a scene is received.' },
    ],
  },

  'misdirection': {
    title: 'Misdirection: how to mislead without cheating',
    category: 'Structure',
    time: '7 min',
    level: 'Intermediate',
    intro: 'Misdirection is not a trick. It is a structural commitment to two truths simultaneously — one visible, one hidden. The difference between misdirection and a cheat is that misdirection plays fair. Everything the audience needs is in the story. They just looked where you pointed.',
    body: [
      { type: 'p', text: 'Every story that surprises an audience is in the misdirection business. The question is whether you are doing it deliberately or accidentally. Accidental misdirection happens when your story is unclear. Deliberate misdirection is when your story is clear — just about the wrong thing. The false lead is not an absence of information. It is an abundance of information pointing in the wrong direction.' },
      { type: 'rule', text: 'A cheat introduces information only at the end. Misdirection plants the true answer from the beginning — but gives the audience a more compelling reason to believe the false one.' },
      { type: 'h3', text: 'The contract' },
      { type: 'p', text: 'Misdirection makes a contract with the audience: I will surprise you, but only with what was always true. When the truth lands, the audience should be able to look back and see every plant that was always pointing there. The revelation should produce the feeling of inevitability — not "that\'s impossible," but "of course, how did I miss it?" Break that contract and the surprise becomes a betrayal. The audience feels cheated because you changed the rules rather than concealing them.' },
      { type: 'p', text: 'This means the false lead must be more emotionally plausible than the true one. The audience chooses the wrong answer not because the right answer is hidden, but because the wrong answer feels right. In The Sixth Sense, Cole\'s behavior is consistent with a troubled child grieving his parents\' separation. That explanation is more familiar, more sympathetic, more emotionally legible than the truth. The audience takes it because it costs less to believe.' },
      { type: 'h3', text: 'Double planting' },
      { type: 'p', text: 'The craft of misdirection requires planting two versions of the truth simultaneously — the false one visibly, the true one buried. Every plant for the hidden truth is also, on its surface, a plant for the false lead. The gun in the drawer is obviously placed there for the confrontation scene you\'re setting up. It\'s also there because the gun was the murder weapon ten years ago and the character knows it. Both things are true. The audience only holds the first.' },
      { type: 'p', text: 'In Gone Girl, Amy\'s diary is careful, specific, intimate. It reads as the record of a real woman\'s deteriorating marriage. It is also — retroactively — a document constructed by a planner with full knowledge of how it would be read. Every detail in it is real evidence for the false version. The same details become evidence for the true version once the structure is revealed. Flynn doesn\'t change what happened — she changes what the events mean.' },
      { type: 'h3', text: 'The fairness test' },
      { type: 'p', text: 'When you have written your ending, apply the fairness test: reread your story knowing the truth. Can you see every plant? Is the hidden truth visible throughout, covered only by the false lead\'s emotional logic? If you cannot find the plants, they are not there — which means you have a cheat, not a twist. Go back and add them, buried in scenes that seem to be about something else entirely.' },
      { type: 'p', text: 'The second part of the test: is the false lead more comfortable than the true one? The wrong answer should be the one the audience wants to be true. A mystery where the audience suspects the right person from page one is not misdirection — it is a slow reveal. Misdirection requires that the false answer have emotional weight. The audience should feel, when the truth lands, that they were complicit in their own deception. They wanted the wrong answer.' },
      { type: 'h3', text: 'Where misdirection lives' },
      { type: 'p', text: 'Plot misdirection (false suspects, fake clues) is the most obvious kind. But misdirection also lives in character: the antagonist who appears to be a friend, the ally who appears to be an enemy, the character whose apparent motivation conceals the real one. And in genre: the horror film that looks like a domestic drama until it doesn\'t, the thriller whose first act reads as a love story. These genre misregistrations set expectations the story then systematically violates — which is misdirection at the largest structural scale.' },
      { type: 'tip', text: 'Write your ending first. Establish the hidden truth. Then plant two paths through your story — the false lead visibly, the true one buried. For each scene, ask: what does this scene seem to be about? What is it actually about? If the answers are always the same, you have no misdirection. The scenes that seem to be about one thing while being about another are the ones that will survive the reread.' },
    ],
  },

  'the-reveal': {
    title: 'The reveal: how to earn a disclosure',
    category: 'Structure',
    time: '7 min',
    level: 'Intermediate',
    intro: 'A reveal is a structural event. It is the moment when hidden information becomes visible — and changes the meaning of everything the audience thought they understood. Not all reveals are twists. A twist changes what happened. A reveal changes what it meant.',
    body: [
      { type: 'p', text: 'The difference between a reveal and an exposition dump is retroactivity. An exposition dump tells you new information that affects what happens next. A reveal tells you information that recontextualizes everything that already happened. When Chinatown reaches its ending, the film doesn\'t add a new story — it reveals the story you were watching was entirely different from the one you thought you were watching. That retroactive effect is what separates a true revelation from a plot complication.' },
      { type: 'rule', text: 'A reveal that only changes what comes next is information. A reveal that changes what already happened is a revelation. The difference is whether the audience has to reassemble everything they thought they understood.' },
      { type: 'h3', text: 'Three conditions' },
      { type: 'p', text: 'For a reveal to land, three things have to be true. The information must change the meaning of what came before, not just what comes after. The evidence must have been available — in the story, visible to any audience paying close attention — even if it was never assembled into the right conclusion. And the character who receives the revelation must be permanently changed by it. A reveal that leaves the receiver unchanged has either been mishandled or misplaced.' },
      { type: 'p', text: 'The second condition is the one most often violated. Writers know their ending. They forget to plant the evidence that makes it fair. Go back to your story and check: if you removed the revelation entirely, is the hidden truth still visible in the preceding story? It should be. Not obviously — but visibly, to an audience that knows where to look. This is not easy to do. It requires planting information that must look like something else until the reveal retroactively changes its meaning.' },
      { type: 'h3', text: 'Timing the revelation' },
      { type: 'p', text: 'Reveals land harder at moments of maximum emotional investment. The audience must have something to lose for the truth to cost them. A character revealed to be a traitor in the first act is a plot setup. The same character revealed in the final act — after the audience has watched them risk their life for the protagonist, after trust has been earned, after attachment has formed — produces an entirely different response. The information is identical. The investment is not.' },
      { type: 'p', text: 'This is why Act Three is the natural home of most revelations: the audience has spent the entire story building an understanding that the reveal dismantles. The longer they have held the false understanding, the greater the impact of its collapse. But timing is not just position in the story — it is position within the scene. A reveal that arrives too early in a scene allows the character (and the audience) to process and adapt before the scene ends. A reveal in the last moment of a scene forces the audience to carry the unprocessed information into whatever comes next. That carrying is where the reveal does its work.' },
      { type: 'h3', text: 'The false reveal' },
      { type: 'p', text: 'Some stories give a fake revelation before the true one. The detective names the wrong killer. The spy thinks they\'ve identified the mole. The lover finds what appears to be evidence of betrayal. The false reveal allows the story to do two things simultaneously: provide the emotional experience of a revelation while keeping the true information hidden, and set up the second, true revelation to land harder because the audience has already processed their assumptions once and believe they are now past the surprise.' },
      { type: 'p', text: 'The false reveal is a high-risk technique. If the audience identifies it as false too quickly, it reads as a cheat. If it holds too long, the true revelation arrives too close to the ending to breathe. The best false reveals are the ones the audience suspects but cannot confirm — which means maintaining competing evidence for both the false and the true version throughout the period between them.' },
      { type: 'h3', text: 'Character revelation versus plot revelation' },
      { type: 'p', text: 'The most lasting reveals are about character, not plot. Learning the murderer\'s name is a plot revelation. Learning why they believed it was justified — the specific logic that made violence feel like the only option — is a character revelation. Plot revelations close questions. Character revelations open them. The best stories use the plot revelation as a delivery mechanism for the character revelation: you find out who did it at the same moment you understand why, and why hits harder.' },
      { type: 'tip', text: 'For every major reveal in your story, ask two questions. What does the audience believe before this moment? What do they believe after? If both answers are the same, the reveal revealed nothing — it confirmed what they already suspected. And: does the reveal change the meaning of specific earlier scenes? Name them. If you can\'t, the information may be new but the revelation is not.' },
    ],
  },

  'escalation': {
    title: 'Escalation: how stories raise the pressure',
    category: 'Structure',
    time: '6 min',
    level: 'Intermediate',
    intro: 'Escalation is not the act of adding bigger problems. It is the act of making problems more personal. Not more spectacular — more costly. The difference between a story that builds unbearable pressure and one that exhausts the audience is this distinction.',
    body: [
      { type: 'p', text: 'Adding a larger explosion is not escalation. It is spectacle. Spectacle can exist without escalation — you can put an audience through increasingly impressive set pieces and leave them strangely unmoved. This happens because spectacle is about what the world does. Escalation is about what the world costs a specific person. A story escalates when each new obstacle is harder to survive not because it is bigger, but because it takes something from the character that cannot be replaced.' },
      { type: 'rule', text: 'The ratchet only turns one way. Each reversal must leave the character in a worse position than the last. If they can fully recover between obstacles, the pressure has been released. You have to keep it engaged.' },
      { type: 'h3', text: 'Three axes' },
      { type: 'p', text: 'Effective escalation moves on three axes simultaneously, and stories that only escalate on one get exhausting. The external axis is what the world does: the threat grows, the resources shrink, the time runs out. The internal axis is what the character\'s own flaws cost them: every bad decision compounds the previous one, every attempt at a shortcut creates a new problem, the character\'s wound keeps making the external problem worse. The relational axis is what the conflict destroys between people: the ally who can no longer trust the protagonist, the relationship that cannot return to what it was, the person the character hurts in the process of trying to save themselves.' },
      { type: 'p', text: 'A story that only escalates externally produces an action movie without stakes. A story that only escalates internally produces a character study without drive. The most unbearable stories hit all three axes and make them reinforce each other — the protagonist\'s flaw damages the relationship that was their last support, which forces them into a position where the external threat becomes unsolvable. The three axes converge.' },
      { type: 'h3', text: 'Yes, but / No, and' },
      { type: 'p', text: 'A writing technique for testing whether your story is escalating: apply the "yes, but / no, and" test to every scene. Does the character succeed at their immediate goal? If yes, a complication should arrive (yes, but). Does the character fail? The failure should make things worse, not just harder (no, and). "No, but it could have been worse" releases pressure. "No, and now one more thing is lost" engages the ratchet. If your story has too many "yes, and" scenes — the character succeeds and the situation improves — you have written a travelogue, not a drama.' },
      { type: 'h3', text: 'The midpoint as escalation hinge' },
      { type: 'p', text: 'Before the midpoint, the protagonist is largely reactive — the world is happening to them and they are responding. After the midpoint, something changes in how they engage. They become active. They commit. They decide to pursue rather than flee. This shift changes the nature of the escalation: the same external pressure now has a character running toward it instead of away, which means their choices begin to drive the escalation rather than just experiencing it. Their agency creates the pressure in the second half. This is why the midpoint is an escalation hinge — the story\'s intensification changes source.' },
      { type: 'h3', text: 'The irreversible cost' },
      { type: 'p', text: 'The strongest escalation moments are the ones that cannot be undone. Not setbacks — permanent costs. Someone who cannot be un-hurt. Something that cannot be un-said. A relationship that cannot return to what it was. A door the character walked through that has closed behind them. These irreversible losses accumulate across the story, and the audience carries each one. By the climax, the weight of everything already lost is part of what makes the final confrontation feel so high. The stakes are not just what the character might lose — they are what the character has already lost trying to get here.' },
      { type: 'tip', text: 'List your protagonist\'s five most important relationships and five most important beliefs about themselves. Now map which ones are threatened in each act. Escalation means these threats become more personal and more costly over time, until the climax puts the last one on the line. If a relationship or belief is under threat in Act One and fully resolved by Act Two, it was a subplot, not an escalation axis. Keep the pressure engaged until the end.' },
    ],
  },
