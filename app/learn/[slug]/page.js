...
const lessons = {
  'what-a-scene-does': {
    ...
    body_academic: [
      { type: 'p', text: "Aristotle, in the Poetics, identified the scene (episodion) as the basic unit of mimetic action — a discrete, bounded event that moves the dramatic situation from one state to another. The scene is not merely a container for dialogue or spectacle; it is a functional unit that must alter the story's trajectory to justify its presence." },
      { type: 'rule', text: "Aristotle's criterion for scene inclusion: necessity and probability. A scene that can be removed without disturbing the causal chain should not exist." },
      { type: 'h3', text: "Swain's scene-sequel model" },
      { type: 'p', text: "Dwight Swain formalized the scene-sequel distinction in Techniques of the Selling Writer (1965). The scene operates on the MRU (Motivation-Reaction Unit): external stimulus produces goal-directed action that terminates in disaster. The sequel processes disaster through emotion, thought, and a new decision. The alternation of scene and sequel is the fundamental rhythm of dramatic prose." },
      { type: 'h3', text: "Multi-function analysis" },
      { type: 'p', text: "Contemporary analysts including Linda Seger (Making a Good Script Great, 1987) and Ken Dancyger (The Technique of Film and Video Editing, 1993) argue that professional-grade scenes must operate on at least two functional levels simultaneously: plot progression and character revelation, or thematic development and relational deepening. The scene serving only one function is, by this standard, an exposition unit rather than a dramatic unit." },
      { type: 'tip', text: "Apply the two-function test: what does the scene do for the external plot, and what does it do for the internal character arc? If only one answer emerges, the scene requires revision or removal." },
    ],
  },
  'midpoint': {
  ...
  'want-vs-need': {
    ...
    body_academic: [
      { type: 'p', text: "The distinction between conscious desire and unconscious need has been theorized by K.M. Weiland (Creating Character Arcs, 2016) as the fundamental engine of the character arc. The external want supplies narrative propulsion; the internal need supplies thematic depth." },
      { type: 'rule', text: "McKee (Story, 1997) designates want as 'conscious desire' and need as the character's deeper requirement for psychological completion. Satisfying the need constitutes the arc's resolution." },
      { type: 'h3', text: "Aristotelian hamartia and the need structure" },
      { type: 'p', text: "Aristotle's hamartia — often mistranslated as 'fatal flaw' — is more precisely a misapprehension: the protagonist acts under a false understanding of themselves or the world. The want-need split is a structural instantiation of hamartia. The protagonist pursues the wrong goal (want) because of a fundamental misapprehension about what will make them whole (need). The arc corrects this misapprehension." },
      { type: 'h3', text: "Structural implications" },
      { type: 'p', text: "In three-act narrative, the want drives the A story (external plot) and the need drives the B story (thematic subplot). Their convergence at the Act 2 break — the moment the protagonist must choose between want and need — constitutes the story's central argument. Choosing need produces a positive arc. Choosing want over need produces tragedy." },
      { type: 'tip', text: "Identify which choice your protagonist makes at the climax: want or need? The answer defines the story's moral argument." },
    ],
  },
  'dialogue-subtext': {
  ...
  'theme': {
    ...
    body_academic: [
      { type: 'p', text: "Aristotle distinguished dianoia (thought, argument, theme) from mythos (plot) and ethos (character). Dianoia operates through what characters say and the values their actions dramatize — not through explicit statement. The Aristotelian model insists that theme is always embedded in action, never announced." },
      { type: 'rule', text: "McKee's formulation: the controlling idea is a single sentence expressing how and why life changes at the story's climax. It has two components — a value (positive or negative) and a cause (the force that produces that change). 'Justice prevails because the individual refuses to abandon principle' is a controlling idea." },
      { type: 'h3', text: "Brecht vs. Aristotle on thematic transparency" },
      { type: 'p', text: "Bertolt Brecht's Verfremdungseffekt (alienation effect) represents a deliberate rupture with Aristotelian thematic embedding. Where Aristotle conceals theme inside action, Brecht surfaces it — using placards, direct address, and interrupted narrative to force the audience into analytical rather than empathic engagement with the thematic argument. Most commercial narrative follows the Aristotelian model; much of prestige television and art cinema borrows from the Brechtian." },
      { type: 'h3', text: "Theme as structural constraint" },
      { type: 'p', text: "Screenwriting theorists including John Truby (The Anatomy of Story, 2007) argue that theme is not ornamental — it is structurally generative. The thematic argument determines what kind of antagonist is required (a force that embodies the counter-argument), what the protagonist's ghost must be (the wound that produces the false belief), and how the climax must function (as the story's proof or refutation of the central proposition)." },
      { type: 'tip', text: "State your controlling idea as McKee prescribes: '[positive or negative value] because [causal force].' Then check whether your antagonist embodies the opposing idea. If not, there is a structural inconsistency in your thematic architecture." },
    ],
  },
  'act-breaks': {
    ...
    body_academic: [
      { type: 'p', text: "Aristotle identified peripeteia (reversal) and anagnorisis (recognition) as the two structural mechanisms capable of generating the most powerful dramatic effects. Act breaks are the primary sites of peripeteia in three-act structure — moments at which the dramatic situation reverses, permanently altering the protagonist's trajectory." },
      { type: 'rule', text: "Syd Field (Screenplay, 1979) codified the act break as a 'plot point': an incident, episode, or event that hooks into the story and spins it in a new direction. The spin is non-negotiable — a moment that merely intensifies without redirecting is not a plot point." },
      { type: 'h3', text: "The threshold function of Act 1 break" },
      { type: 'p', text: "Joseph Campbell's threshold crossing — the moment the hero leaves the ordinary world — maps directly onto the Act 1 break. Campbell's structural insight, extended into three-act screenwriting by Christopher Vogler (The Writer's Journey, 1992), is that the threshold must be crossed volitionally. The protagonist who is pushed across, rather than choosing to cross, forfeits dramatic agency — and the audience's identification." },
      { type: 'h3', text: "Synthesis at the Act 2 break" },
      { type: 'p', text: "McKee identifies the Act 2 break as the point of maximum crisis — the 'obligatory scene' toward which all prior causality has been driving. Snyder's 'Break into Three' reframes this as a synthesis event: the A and B story threads converge, and the protagonist integrates what they have learned from both. This synthesis is not merely emotional — it is epistemic. The protagonist now knows something they did not know before, and that knowledge makes Act 3 action possible." },
      { type: 'tip', text: "Analyze whether your Act 1 break involves protagonist choice (agency) or external compulsion (passivity). If compulsion, ask: what small adjustment makes the protagonist an active participant in their own transition into Act 2?" },
    ],
  },
  'all-is-lost': {
    ...
    body_academic: [
      { type: 'p', text: "The All Is Lost beat corresponds to what Campbell called the 'belly of the whale' — the moment of full immersion in crisis, the symbolic death that precedes rebirth. In Jungian terms, it is the encounter with the shadow in its most overwhelming form: the moment the ego's defenses have completely failed." },
      { type: 'rule', text: "Snyder's formulation: the All Is Lost beat must include a 'whiff of death' — the loss of something irreplaceable. Without genuine loss, the subsequent Break into Three carries no emotional weight, because there is nothing to be reborn from." },
      { type: 'h3', text: "The katabasis tradition" },
      { type: 'p', text: "The All Is Lost beat is the narrative descendant of katabasis — the descent into the underworld present in the Epic of Gilgamesh, the Odyssey, Virgil's Aeneid, and Dante's Inferno. The hero must descend to the lowest possible point before the ascent becomes possible. The structural function of katabasis is identical across 4,000 years of narrative: the protagonist must be emptied before they can be filled with new understanding." },
      { type: 'h3', text: "Distinguishing All Is Lost from Dark Night" },
      { type: 'p', text: "Snyder's structural distinction between All Is Lost (the event of loss) and Dark Night of the Soul (the emotional processing of loss) maps onto Aristotle's distinction between action and passion. All Is Lost is what happens. Dark Night is what is suffered. Both are required because catharsis — the purgation Aristotle describes as tragedy's purpose — requires that the audience experience both the event and its emotional aftermath." },
      { type: 'tip', text: "Identify the specific object of loss in your All Is Lost beat. Is it external (a person, a goal, a resource) or internal (an identity, a belief, a hope)? The most powerful All Is Lost beats involve loss on both registers simultaneously." },
    ],
  },
  'ghost': {
